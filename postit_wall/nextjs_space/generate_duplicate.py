...
settings_end = content.find('  </TabsContent>', settings_start)
settings_content = content[settings_start:settings_end]

# Extract the inner part of {wallForm.isOttActive && ( ... )}
# We know the opening is `{wallForm.isOttActive && (\n                    <div className="space-y-4 border p-4 rounded-md bg-gray-50">`
# and the closing is `                  )}`
# Let's find exactly the `)}` that matches the condition. Actually we can just string replace `wallForm.isOttActive` to `wallForm.isStyleModeActive`!
