...
tar_name = 'update-message-button.tar.gz'
...
try:
    ...
    child_ssh.sendline(f"cd {remote_dir} && rm -f components/forms/admin-message-fab.tsx")
    child_ssh.expect('#', timeout=30)

    print("Removing .next cache...")
    child_ssh.sendline(f"cd {remote_dir} && rm -rf .next/cache")
    child_ssh.expect('#', timeout=30)
