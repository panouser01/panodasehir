...
export function ArticleShareButton({ articleId, categoryId, className, iconClassName = "w-4 h-4 md:w-5 md:h-5", initialShareCount = 0, onShare }: ArticleShareButtonProps) {
  const [isOpen, setIsOpen] = useState(false);
  const [shareCount, setShareCount] = useState(initialShareCount);
  
  // URL to share the specific article
  const shareUrl = typeof window !== 'undefined' ? `${window.location.origin}/?category=${categoryId}&article=${articleId}` : '';
  ...
  const handleShare = (e: React.MouseEvent, platform: string) => {
    ...
    switch (platform) {
      ...
      case 'linkedin':
        ...
        break;
      case 'email':
        url = `mailto:?subject=${encodeURIComponent('Panoda Şehir Paylaşımı')}&body=${encodeURIComponent('Bu makaleye bir göz at: ' + shareUrl)}`;
        break;
      case 'instagram':
  ...
  const executeShareAction = () => {
    ...
    if (onShare) {
      ...
      fetch(`/api/articles/${articleId}/interact`, { 
        method: 'POST',
      ...
      <Popover open={isOpen} onOpenChange={setIsOpen}>
        <PopoverTrigger asChild>
          <div 
            ...
            className={`flex items-center gap-1.5 cursor-pointer transition-colors hover:text-indigo-500 z-50 relative pointer-events-auto ${className || ''}`}
            title="Paylaş"
          >
            <Share2 className={iconClassName} />
            <span className="font-bold text-sm">{shareCount}</span>
          </div>
        </PopoverTrigger>
