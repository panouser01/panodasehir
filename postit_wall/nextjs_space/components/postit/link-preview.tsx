...
function InstagramScaledFallback({ url }: { url: string }) {
  ...
  return (
    <div 
      ref={containerRef} 
      className="w-full h-full bg-white rounded-lg overflow-hidden relative shadow-sm border border-gray-200 pointer-events-none flex items-start justify-center"
    >
       <div 
...
export function CustomLinkPreview({ url, compact = false, fill = false, hideIcon = false }: { url: string, compact?: boolean, fill?: boolean, hideIcon?: boolean }) {
  ...
  if (isInstagram && instagramPath) {
    if (fill) {
      // Her zaman gerçek Instagram widget'ını (InstagramScaledFallback) kullanarak 
      // Microlink'in 1:1 kırptığı resmi es geçiyoruz, böylece orantı bozulmuyor.
