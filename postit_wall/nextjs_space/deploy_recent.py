...
for file in files:
    ...
    scp = pexpect.spawn(f"scp -P {port} -o StrictHostKeyChecking=no '{file}' {target}:/var/www/panodasehir/{escaped_file}", encoding='utf-8')
    scp.expect('ssword:', timeout=15)
    scp.sendline(password)
    scp.expect(pexpect.EOF, timeout=120)

print("Building and restarting via SSH...")
