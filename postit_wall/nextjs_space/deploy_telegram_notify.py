...
print("Uploading telegram.ts & comments/route.ts...")
scp = pexpect.spawn(f"scp -P {port} -o StrictHostKeyChecking=no 'lib/telegram.ts' {target}:/var/www/panodasehir/lib/telegram.ts", encoding='utf-8')
scp.expect('ssword:', timeout=15)
scp.sendline(password)
scp.expect(pexpect.EOF, timeout=120)

scp2 = pexpect.spawn(f"scp -P {port} -o StrictHostKeyChecking=no 'app/api/comments/route.ts' {target}:/var/www/panodasehir/app/api/comments/route.ts", encoding='utf-8')
