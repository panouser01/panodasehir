...
files = [
    ...
    ('app/api/settings/route.ts', '/var/www/panodasehir/app/api/settings/route.ts'),
...
print("Building, migrating and restarting server...")
ssh = pexpect.spawn(f'ssh -p {port} -o StrictHostKeyChecking=no {target}', encoding='utf-8')
ssh.logfile = sys.stdout
ssh.expect('ssword:', timeout=15)
ssh.sendline(password)
ssh.expect('# ', timeout=30)
ssh.sendline('cd /var/www/panodasehir && npx prisma db push && npx prisma generate && npm run build')
ssh.expect('# ', timeout=500)
