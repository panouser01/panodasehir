...
scp = pexpect.spawn('scp -P 25416 -o StrictHostKeyChecking=no update-style-2.tar.gz root@45.43.152.18:/var/www/panodasehir/', encoding='utf-8')
...
try:
    ...
    one_liner = "cd /var/www/panodasehir && tar -xzvf update-style-2.tar.gz && npx prisma db push --accept-data-loss && npx prisma generate && npm run build && pm2 restart all"
