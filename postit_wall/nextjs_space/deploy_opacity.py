...
for file in files:
    # Handle brackets in filename for scp destination
    dest_file = file.replace('[', '\\[').replace(']', '\\]')
    print(f"Uploading {file}...")
    scp = pexpect.spawn(f"scp -P {port} -o StrictHostKeyChecking=no '{file}' {target}:/var/www/panodasehir/{dest_file}", encoding='utf-8')
