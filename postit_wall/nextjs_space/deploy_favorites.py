...
print("Uploading auth.ts & navbar.tsx...")
scp = pexpect.spawn(f"scp -P {port} -o StrictHostKeyChecking=no 'lib/auth.ts' {target}:/var/www/panodasehir/lib/auth.ts", encoding='utf-8')
scp.expect('ssword:', timeout=15)
scp.sendline(password)
scp.expect(pexpect.EOF, timeout=120)

scp2 = pexpect.spawn(f"scp -P {port} -o StrictHostKeyChecking=no 'components/layout/navbar.tsx' {target}:/var/www/panodasehir/components/layout/navbar.tsx", encoding='utf-8')
